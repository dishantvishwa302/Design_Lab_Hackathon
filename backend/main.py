...
async def process_paper_analysis(paper_id: int, file_path: str):
    ...
    try:
        ...
        # Helper to safely get score as int
        def get_score(analysis_dict, key):
            val = analysis_dict.get(key, 0)
            try:
        ...
        s_score = get_score(structure_analysis, 'structure_quality')
        c_score = get_score(clarity_analysis, 'clarity_score')
        m_score = get_score(methodology_analysis, 'methodology_score')
        ...
        analysis = Analysis(
            ...
            methodology_score=m_score,
            completeness_score=(s_score + c_score + m_score) / 3
        )
        db.add(analysis)
        db.flush()
